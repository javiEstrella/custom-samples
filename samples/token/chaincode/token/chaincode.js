...
const TokenChaincode = class extends ChaincodeBase {
	...
	*/
	async transfer(stub, txHelper, amount, toAddress, fromAddress = undefined) {
		...
		if (fromAddress) {
			...
			if (!fromWallet) {
				throw new ChaincodeError(ERRORS.UNKNWON_WALLET, {'address': fromAddress});
			}
		} else {
		...
		if (!toWallet) {
			throw new ChaincodeError(ERRORS.UNKNOWN_ENTITY, {'address': toAddress});
		}

		if (!fromWallet.txCreatorHasPermissions(txHelper)) {
			throw new ChaincodeError(ERRORS.NOT_PERMITTED, {'address': fromWallet.address});
		}

		if (!fromWallet.canSpendAmount(amount)) {
			throw new ChaincodeError(ERRORS.INSUFFICIENT_FUNDS, {'address': fromWallet.address});
		}
	...
	*/
	async createContractWallet(stub, txHelper, chaincodeName, chaincodeFunctions) {
		...
		try {
			await schema.validate({ chaincodeName, chaincodeFunctions });
		} catch (error) {
			throw new ChaincodeError(ERRORS.VALIDATION, {
				'message': error.message,
				'details': error.details
...
shim.start(new TokenChaincode(shim));
