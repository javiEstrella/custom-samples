...
class UserWallet extends AbstractWallet {
	...
	static async queryUserWalletFor(txHelper, publicKeyHash) {
		const wallets = await AbstractWallet.queryWallets(txHelper, CONSTANTS.WALLET_TYPES.USER, {
			'properties': {'publicKeyHash': publicKeyHash}
		});
		...
		throw new ChaincodeError(ERRORS.MULTIPLE_WALLETS_FOUND_FOR_USER, {'publicKeyHash': publicKeyHash});
	}
	...
	get properties() {
		return {'publicKeyHash': this.publicKeyHash};
	}
